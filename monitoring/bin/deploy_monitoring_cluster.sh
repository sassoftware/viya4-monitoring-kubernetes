...
# NOTE: Node Exporter runs with hostNetwork: true, so this NetworkPolicy is
# not enforced on all CNIs (confirmed on Calico); actual access control is
# provided by the kube-rbac-proxy RBAC check.
