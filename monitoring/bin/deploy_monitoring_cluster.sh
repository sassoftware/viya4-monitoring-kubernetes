...
if [ "$AUTOGENERATE_INGRESS" == "true" ]; then
   ...
   if [ "$routing" == "host" ]; then
      ...
      yq -i '.alertmanager.ingress.hosts.[0]=env(BASE_DOMAIN)'                "$autogeneratedYAMLFile"
      yq -i '.alertmanager.ingress.tls.[0].hosts.[0]=env(BASE_DOMAIN)'        "$autogeneratedYAMLFile"
      ...
      yq -i '.grafana.ingress.hosts.[0]=env(BASE_DOMAIN)'                     "$autogeneratedYAMLFile"
      yq -i '.grafana.ingress.tls.[0].hosts.[0]=env(BASE_DOMAIN)'             "$autogeneratedYAMLFile"
      slashpath="/$GRAFANA_PATH" yq -i '.grafana.ingress.path=env(slashpath)' "$autogeneratedYAMLFile"
      yq -i '.grafana."grafana.ini".server.domain=env(BASE_DOMAIN)'           "$autogeneratedYAMLFile"
      rooturl="https://$GRAFANA_FQDN" yq -i '.grafana."grafana.ini".server.root_url=env(rooturl)'         "$autogeneratedYAMLFile"

      yq -i '.prometheus.ingress.hosts.[0]=env(BASE_DOMAIN)'                  "$autogeneratedYAMLFile"
      yq -i '.prometheus.ingress.tls.[0].hosts.[0]=env(BASE_DOMAIN)'          "$autogeneratedYAMLFile"
