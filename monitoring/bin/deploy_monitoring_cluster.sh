...
if [ "$AUTOGENERATE_INGRESS" == "true" ]; then
    ...
    if [ "$INGRESS_TYPE" == "ingress-nginx" ]; then
        ...
        ## TO DO:  How to handle AM/PR since those lines are commented out?
        ##         Remove ## as part of file initialization?
        ##         Use yq to delete AM/PR stanzas if app is not enabled?
        ## update: appears to put much of what we need ...but omits some keys we don't update..good enough? what breaks?
        ## BREAKS: * Prometheus.monitoring.coreos.com "v4m-prometheus" is invalid: [spec.alerting.alertmanagers[0].name: Required value, spec.alerting.alertmanagers[0].port: Required value]
        ...
        if [ "$ROUTING" == "host" ]; then
            ...
            # TO DO: Validate that the following stuff is still needed/valid
