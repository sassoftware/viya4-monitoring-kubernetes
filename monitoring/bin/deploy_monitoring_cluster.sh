...
# Single user.env switch for the Grafana AI chatbot: when true, this script
# provisions the MCP servers and LLM objects before the helm install and
# delivers the chatbot plugin after it — no separate script runs needed.
export AI_CHATBOT_PROVISION="${AI_CHATBOT_PROVISION:-false}"
if [ -n "${AI_CHATBOT_ENABLE:-}" ]; then
    log_warn "AI_CHATBOT_ENABLE has been renamed to AI_CHATBOT_PROVISION and is now ignored;"
    log_warn "set AI_CHATBOT_PROVISION=true in user.env to deploy the AI chatbot."
fi
...
if [ "$AUTOGENERATE_INGRESS" == "true" ]; then
    ...
    if [ "$AI_CHATBOT_PROVISION" == "true" ]; then
...
if [ "$AI_CHATBOT_PROVISION" == "true" ]; then
    ...
    monitoring/bin/deploy_ai_chatbot.sh --provision-only
fi
...
if [ "$AI_CHATBOT_PROVISION" == "true" ]; then
    ...
    log_debug "AI_CHATBOT_PROVISION not set; skipping deployment of the Grafana AI chatbot plugin"
fi
