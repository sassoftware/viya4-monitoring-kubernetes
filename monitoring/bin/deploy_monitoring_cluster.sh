...
if [ "$AUTOGENERATE_INGRESS" == "true" ]; then
    ...
    if [ "$INGRESS_TYPE" == "ingress-nginx" ]; then
        ...
        if [ "$ROUTING" == "host" ]; then
            ...
            slashpath="/$ALERTMANAGER_PATH" yq -i '.alertmanager.alertmanagerSpec.routePrefix=env(slashpath)' "$autogeneratedYAMLFile"
            exturl="https://$ALERTMANAGER_FQDN/$ALERTMANAGER_PATH" yq -i '.alertmanager.alertmanagerSpec.externalUrl=env(exturl)' "$autogeneratedYAMLFile"
        ...
        ## tested with sample version: 0.1.0
        ingressSampleFile="samples/gateway-api/${ROUTING}-based/monitoring/user-values-prom-operator.yaml"
        ...
        if [ "$ALERTMANAGER_INGRESS_ENABLE" == "true" ]; then
            create_httproute "monitoring" "alertmanager" "$ALERTMANAGER_PATH" "$ALERTMANAGER_FQDN"
            kubectl -n "$MON_NS" label httproute v4m-alertmanager managed-by="v4m-es-script"
        else
            log_debug "Access to [Alertmanager] disabled"
            kubectl -n "$MON_NS" delete httproute v4m-alertmanager --ignore-not-found
        fi

        if [ "$GRAFANA_INGRESS_ENABLE" == "true" ]; then
            create_httproute "monitoring" "grafana" "$GRAFANA_PATH" "$GRAFANA_FQDN"
            kubectl -n "$MON_NS" label httproute v4m-grafana managed-by="v4m-es-script"
        else
            log_debug "Access to [Grafana] disabled"
            kubectl -n "$MON_NS" delete httproute v4m-grafana --ignore-not-found
        fi

        if [ "$PROMETHEUS_INGRESS_ENABLE" == "true" ]; then
            create_httproute "monitoring" "prometheus" "$PROMETHEUS_PATH" "$PROMETHEUS_FQDN"
            kubectl -n "$MON_NS" label httproute v4m-prometheus managed-by="v4m-es-script"
        else
            log_debug "Access to [Prometheus] disabled"
        ...
        if [ "$INGRESS_BACKEND_TLS_ENABLE" == "true" ]; then
            ...
            if [ "$ALERTMANAGER_INGRESS_ENABLE" == "true" ]; then
                apply_backend_tls_policy "$MON_NS" v4m-alertmanager-backend-tls v4m-alertmanager alertmanager-tls-secret
            else
                kubectl -n "$MON_NS" delete backendtlspolicy v4m-alertmanager-backend-tls --ignore-not-found
            fi

            if [ "$GRAFANA_INGRESS_ENABLE" == "true" ]; then
                apply_backend_tls_policy "$MON_NS" v4m-grafana-backend-tls v4m-grafana grafana-tls-secret
            else
                kubectl -n "$MON_NS" delete backendtlspolicy v4m-grafana-backend-tls --ignore-not-found
            fi

            if [ "$PROMETHEUS_INGRESS_ENABLE" == "true" ]; then
                apply_backend_tls_policy "$MON_NS" v4m-prometheus-backend-tls v4m-prometheus prometheus-tls-secret
            else
                kubectl -n "$MON_NS" delete backendtlspolicy v4m-prometheus-backend-tls --ignore-not-found
            fi
        else
            kubectl -n "$MON_NS" delete backendtlspolicy v4m-alertmanager-backend-tls v4m-grafana-backend-tls v4m-prometheus-backend-tls --ignore-not-found
        fi
    fi
else
