...
autogeneratedYAMLFile="$TMP_DIR/autogenerate-grafana.yaml"
...
if [ "$AUTOGENERATE_STORAGECLASS" == "true" ]; then
    ...
    sc="$storageClass" yq -i '.persistence.storageClassName=env(sc)' "$autogeneratedYAMLFile"
...
if [ "$AUTOGENERATE_SMTP" == "true" ]; then
    ...
    # NOTE: We use the Grafana Helm Chart on OpenShift; therefore the keys here
    #       are slightly different than those used on other Kubernetes distributions
    #       as reflected in the deploy_monitoring_cluster.sh script.
    yq -i '."grafana.ini".smtp.enabled=true' "$autogeneratedYAMLFile"

    value="$smtpHost:$smtpPort" yq -i '."grafana.ini".smtp.host=env(value)' "$autogeneratedYAMLFile"

    value="$smtpFromAddress" yq -i '."grafana.ini".smtp.from_address=env(value)' "$autogeneratedYAMLFile"
    value="$smtpFromName" yq -i '."grafana.ini".smtp.from_name=env(value)' "$autogeneratedYAMLFile"
    ...
    if [ -n "$smtpUserSecret" ] && [ -n "$(kubectl get secret -n "$MON_NS" "$smtpUserSecret" -o name 2> /dev/null)" ]; then
        ...
        value="$smtpUserSecret" yq -i '.smtp.existingSecret=env(value)' "$autogeneratedYAMLFile"
    fi

    value="$smtpSkipVerify" yq -i '."grafana.ini".smtp.skip_verify=env(value)' "$autogeneratedYAMLFile"

    value="$smtpTLSCertFile" yq -i '."grafana.ini".smtp.cert_file=env(value)' "$autogeneratedYAMLFile"
    value="$smtpTLSKeyFile" yq -i '."grafana.ini".smtp.key_file=env(value)' "$autogeneratedYAMLFile"
