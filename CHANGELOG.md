...
* **Monitoring**
  * The `KubeHpaMaxedOut` alert has been modified to only trigger if the
  max replicas is > 1
* **Logging**
  * Some new visualizations are available in Kibana
  * Fix timing issues with Elasticsearch security config scripts
  * Fix some issues with upgrade-in-place and re-running deployment scripts
  * Support configurable Docker root
  * Refactored scripts for individual components
