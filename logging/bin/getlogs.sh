...
function show_usage {
   ...
   log_info  "     -po, --pod         POD       - The pod for which logs are sought"
   ...
   log_info  "     -h,  --host        hostname  - Hostname for connection to Elasticsearch/Kibana"
...
if [[ !  -z "$query_file" ]]; then
   ...
   if [ "$OPS_INDEX" == "true" ]; then
      ...
      index_date="2021.02.18"
      index_name="viya_ops-$index_date";
   else
      index_date=$(end_date  +"%Y-%m-%d")
