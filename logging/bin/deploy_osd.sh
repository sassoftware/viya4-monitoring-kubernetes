...
if [ "$AUTOGENERATE_INGRESS" == "true" ] && [ "$OSD_INGRESS_ENABLE" == "true" ]; then
   ...
   ## tested with sample version: 0.3.1
   ...
   if [ "$ROUTING" == "host" ]; then
      ...
      slashpath="/$OSD_PATH" yq -i '(.extraEnvs.[] | select(has("name")) | select(.name == "SERVER_BASEPATH")).value=env(slashpath)' $autogeneratedYAMLFile           

      yq -i '.ingress.hosts.[0].host=env(OSD_FQDN)'            $autogeneratedYAMLFile
      yq -i '.ingress.tls.[0].hosts.[0]=env(OSD_FQDN)'         $autogeneratedYAMLFile
      newpath="/$OSD_PATH(/|$)(.*)" yq -i '.ingress.hosts.[0].paths.[0].path=env(newpath)' $autogeneratedYAMLFile     
   fi
else
   log_debug "Autogeneration of ingresss NOT enabled and/or ingress NOT enabled for OpenSearch Dashboards"
fi
