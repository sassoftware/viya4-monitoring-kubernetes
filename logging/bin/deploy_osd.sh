...
if [ "$AUTOGENERATE_INGRESS" == "true" ] && [ "$OSD_INGRESS_ENABLE" == "true" ]; then
   ...
   # osdIngressCert="${OSD_INGRESS_CERT:-$v4mIngressCert}"
   # osdIngressKey="${OSD_INGRESS_KEY:-$v4mIngressKey}"
   # if [ -f "$osdIngressCert" ] && [ -f "$osdIngressKey" ]; then
   #    kubectl create secret tls kibana-ingress-tls-secret --namespace "$LOG_NS" --key="$osdIngressKey" --cert="$osdIngressCert" 
   # elif [ ! -z "$osdIngressCert$osdIngressKey" ]; then
   #    log_warn "Missing Ingress certificate file; specified Ingress cert [$osdIngressCert] and/or key [$osdIngressKey] file is missing."
   #    log_warn "Create the missing Kubernetes secrets after deployment; use command: kubectl -create secret tls kibana-ingress-tls-secret --namespace $LOG_NS --key=cert_key_file --cert=cert_file"
   # fi

   ## tested with sample version: 0.2.1
   ingressSampleFile="samples/ingress/${ingressRouting}-based-ingress/logging/user-values-osd.yaml"
