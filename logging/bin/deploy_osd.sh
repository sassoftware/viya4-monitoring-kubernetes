...
if [ "$AUTOGENERATE_INGRESS" == "true" ] && [ "$OSD_INGRESS_ENABLE" == "true" ]; then
    ...
    targetPath="${OSD_PATH:-dashboards}"
    targetFqdn="$(get_app_ingress_fqdn "$OSD_FQDN" "$targetPath")"
    export targetFqdn targetPath
    ...
    if [ "$INGRESS_TYPE" == "ingress-nginx" ]; then
        ...
        if [ "$ROUTING" == "host" ]; then
            yq -i '.ingress.hosts.[0].host=strenv(targetFqdn)' "$autogeneratedYAMLFile"
            yq -i '.ingress.tls.[0].hosts.[0]=env(targetFqdn)' "$autogeneratedYAMLFile"
        else

            export slashpath="/$targetPath"

            yq -i '(.extraEnvs.[] | select(has("name")) | select(.name == "SERVER_BASEPATH")).value=env(slashpath)' "$autogeneratedYAMLFile"

            yq -i '.ingress.hosts.[0].host=env(targetFqdn)' "$autogeneratedYAMLFile"
        ...
        if [ "$ROUTING" == "path" ]; then
            ...
            snippet="/$targetPath" yq -i '(.extraEnvs.[] | select(has("name")) | select(.name == "SERVER_BASEPATH")).value=env(snippet)' "$autogeneratedYAMLFile"
        else
            echo "#No content added for auto-generation of ingress" >> "$autogeneratedYAMLFile"
        fi
        ...
        if [ "$ROUTING" == "path" ]; then
            ...
            ingressSampleFile="samples/gateway-api/path-based/logging/user-values-osd.yaml"
        ...
        create_httproute "logging" "osd" "$targetPath" "$targetFqdn"
        kubectl -n "$LOG_NS" label httproute v4m-osd managed-by="v4m-es-script"

        if [ "$INGRESS_BACKEND_TLS_ENABLE" == "true" ]; then
            create_backend_tls_ca_configmap "$LOG_NS"
            apply_backend_tls_policy "$LOG_NS" v4m-osd-backend-tls v4m-osd kibana-tls-secret
        else
            kubectl -n "$LOG_NS" delete backendtlspolicy v4m-osd-backend-tls --ignore-not-found
        fi
    fi
    ...
    kubectl -n "$LOG_NS" delete httpproxy v4m-osd --ignore-not-found

elif [ "$AUTOGENERATE_INGRESS" == "true" ] && [ "$OSD_INGRESS_ENABLE" = "false" ] && [ "$INGRESS_TYPE" == "gateway-api" ]; then
    log_debug "Access to [OpenSearch Dashboards] disabled; removing HTTPRoute resource"
    kubectl -n "$LOG_NS" delete httproute v4m-osd --ignore-not-found
    kubectl -n "$LOG_NS" delete backendtlspolicy v4m-osd-backend-tls --ignore-not-found
...
helm_values_yaml="logging/opensearch/osd_helm_values.yaml"
if [ "$IPV6_ENABLE" == "true" ]; then

    cp "$helm_values_yaml" "$TMP_DIR/osd_helm_values.yaml"
    helm_values_yaml="$TMP_DIR/osd_helm_values.yaml"
