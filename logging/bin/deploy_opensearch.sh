...
if [ "$AUTOGENERATE_INGRESS" == "true" ] && [ "$OPENSEARCH_INGRESS_ENABLE" = "true" ]; then
    ...
    osIngressSecret="${OPENSEARCH_INGRESS_SECRET:-elasticsearch-ingress-tls-secret}"

    create_ingress_certs "$LOG_NS" "$osIngressSecret" "$osIngressCert" "$osIngressKey"
    ...
    OPENSEARCH_FQDN="$(get_app_ingress_fqdn $OPENSEARCH_FQDN $OPENSEARCH_PATH)"
    ...
    if [ "$INGRESS_TYPE" == "ingress-nginx" ]; then
        ...
        ## tested with sample version: 0.0.1
        :
        # NOTE: do nothing here; only action item:
        # create HTTPProxy resources (handled elsewhere)
