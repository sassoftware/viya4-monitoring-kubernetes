...
if [ "$AUTOGENERATE_INGRESS" == "true" ]; then
   ...
   yq -i '.ingress.enabled= env(OPENSEARCH_INGRESS_ENABLED)'             $autogeneratedYAMLFile
   ...
   if [ "$routing" == "host" ]; then
      ...
      yq -i '.ingress.hosts.[0]=env(BASE_DOMAIN)'                        $autogeneratedYAMLFile

      yq -i '.ingress.tls.[0].hosts.[0]=env(BASE_DOMAIN)'                $autogeneratedYAMLFile
...
generateStorageClassReference "opensearch" "$autogeneratedYAMLFile"
