...
if [ "$AUTOGENERATE_INGRESS" == "true" ] && [ "$OPENSEARCH_INGRESS_ENABLE" = "true" ]; then
    ...
    if [ "$INGRESS_USE_SEPARATE_CERTS" == "true" ]; then
        ...
        create_ingress_certs "$LOG_NS" "$ingress_tls_secret" "$INGRESS_CERT" "$INGRESS_KEY"
    fi

    ROUTING="${ROUTING:-host}"

    targetPath="${OPENSEARCH_PATH:-search}"
    targetFqdn="$(get_app_ingress_fqdn "$OPENSEARCH_FQDN" "$targetPath")"

    INGRESS_TYPE="${INGRESS_TYPE:-ingress-nginx}"

    log_debug "INGRESS_TYPE [$INGRESS_TYPE] OPENSEARCH_INGRESS_ENABLE [$OPENSEARCH_INGRESS_ENABLE] OPENSEARCH_FQDN [$targetFqdn] OPENSEARCH_PATH [$targetPath]"
    export OPENSEARCH_INGRESS_ENABLE targetFqdn targetPath
    ...
    if [ "$INGRESS_TYPE" == "ingress-nginx" ]; then
        ...
        yq -i '.ingress.enabled="true"' "$autogeneratedYAMLFile"

        snippet="$ingress_tls_secret" yq -i '.ingress.tls.[0].secretName=strenv(snippet)' "$autogeneratedYAMLFile"

        if [ "$ROUTING" == "host" ]; then
            snippet="$targetFqdn" yq -i '.ingress.hosts.[0]=strenv(snippet)' "$autogeneratedYAMLFile"
            snippet="$targetFqdn" yq -i '.ingress.tls.[0].hosts.[0]=strenv(snippet)' "$autogeneratedYAMLFile"
        else
            slashpath="/$targetPath" yq -i '.ingress.path=strenv(slashpath)' "$autogeneratedYAMLFile"
            snippet="$targetFqdn" yq -i '.ingress.hosts.[0]=strenv(snippet)' "$autogeneratedYAMLFile"

            snippet="$targetFqdn" yq -i '.ingress.tls.[0].hosts.[0]=strenv(snippet)' "$autogeneratedYAMLFile"
            slashpath="/$targetPath" yq -i '.ingress.annotations["nginx.ingress.kubernetes.io/rewrite-target"]=env(slashpath)' "$autogeneratedYAMLFile"

            # Need to use printf to preserve newlines
            printf -v snippet "rewrite (?i)/%s/(.*) /\$1 break;\nrewrite (?i)/%s$ / break;" "$targetPath" "$targetPath"
...
helm_values_yaml="logging/opensearch/opensearch_helm_values.yaml"
if [ "$IPV6_ENABLE" == "true" ]; then

    cp "$helm_values_yaml" "$TMP_DIR/opensearch_helm_values.yaml"
    helm_values_yaml="$TMP_DIR/opensearch_helm_values.yaml"
