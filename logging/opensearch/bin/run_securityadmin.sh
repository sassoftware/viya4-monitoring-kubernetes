...
export ES_PLUGINS_DIR=/usr/share/opensearch/plugins
export ES_CONF_DIR=/usr/share/opensearch/config
...
while [ $rc != 0 ] && [ $counter -lt 3 ]; do
  ...
  $ES_PLUGINS_DIR/opensearch-security/tools/securityadmin.sh -cd "$ES_PLUGINS_DIR/opensearch-security/securityconfig" -icl -key "$ES_CONF_DIR/admin-key.pem" -cert "$ES_CONF_DIR/admin-crt.pem" -cacert "$ES_CONF_DIR/admin-root-ca.pem" -nhnv -rev >> $logfile
  rc=$?
  echo "RC: $rc"
  ((counter++))
done
