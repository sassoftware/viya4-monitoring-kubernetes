...
These scenarios differ in how the URL for the web applications is constructed:
...
using ingress-nginx ingress controller to using Contour is straightforward
and does not require a complete removal and redeployment.
...
existing Ingress resources are removed and new HTTPProxy resources are generated.
* The new URLs needed to access the web applications are shown in the notices genereated
...
using ingress-nginx and now you want to manually configure things for Contour, refer the rest
of this sample for details.  Essential points to remember are:
* Kubernetes Ingress resources are not used when using Contour.  Therefore, you need to
disable the creation of these resources.  Depending on how you previously configured ingress,
this can be done by simply setting the appropriate `ingress.enable` key to "*false*", or removing
...
existing Ingress resources are removed and new HTTPProxy resources are generated.
* The new URLs needed to access the web applications are shown in the notices generated
at the end of the deployment process.
